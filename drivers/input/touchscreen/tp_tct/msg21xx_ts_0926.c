...
char *fw_version; /* customer firmware version*/
#else
...
#ifdef MSTAR_PWRON_UPGRADE
static unsigned char (*temp)[33][1024];
#else
...
#if defined(MSTAR_TP_GESTURE)
int write_i2c_seq(unsigned char addr, unsigned char *buf, unsigned short size)
#else
...
#if defined(MSTAR_TP_GESTURE)
int get_customer_firmware_version(void)
#else
...
#endif
{
	...
	snprintf(fw_version, 30, "%03d%03d", fw_version_major, fw_version_minor);
#else
...
static ssize_t firmware_update_c33(struct device *dev,
	...
	*/
	...
	for (i = 0; i < 32; i++) {
		...
		for (j = 0; j < 8; j++)
#ifdef MSTAR_PWRON_UPGRADE
			write_i2c_seq(ts_data->client->addr, &(*temp)[i][j*128], 128 );
#else
...
#ifdef MSTAR_PWRON_UPGRADE
if (NULL == temp)
	temp = (unsigned char (*)[33][1024]) kzalloc(33 * 1024, GFP_KERNEL);
...
#ifdef MSTAR_PWRON_UPGRADE
int mstar_fw_upgrade_start(const u8 *data, u32 data_len, bool force)
{
    ...
    if (33*1024 == data_len) {
    	MSG_FIRMWARE = (unsigned char (*)[1][33*1024])data;
    } else {
    	printk("%s line=%d, data_len isn't 33KB !!! \n", __func__, __LINE__);
		enable_irq(ts_data->client->irq);
		bFwUpdating = 0;
    	return -1;
    }
    ...
    if (main_sw_id == info_sw_id)
    {
        if (_CalMainCRC32() == bin_conf_crc32)
        {
            ...
            printk("MAIN sw_id=%d,update_flag=%d,update_bin_major=%d,update_bin_minor=%d\n",main_sw_id,update_flag,update_bin_major,update_bin_minor);
        }
        else
        {
            ...
            printk("INFO1 sw_id=%d,update_flag=%d,update_bin_major=%d,update_bin_minor=%d\n",info_sw_id,update_flag,update_bin_major,update_bin_minor);
        }
    }
    else
    {
        ...
        printk("INFO2 sw_id=%d,update_flag=%d,update_bin_major=%d,update_bin_minor=%d\n",info_sw_id,update_flag,update_bin_major,update_bin_minor);
    }
...
#ifdef MSTAR_TP_GESTURE
	if ( (ts_data->gesture_id > 0) && (0x01 == ts_data->gesture_set) ) {
		int ret = 0;
		ret = mstar_tp_interrupt( ts_data, checksum, data);
		return ret;
	}
#endif
...
static irqreturn_t msg21xx_ts_interrupt(int irq, void *dev_id)
{
	...
	if (0 == parse_info(&info)) {
		...
		if ((info.count ) == 0)   //touch end
		{
			if (last_count != 0)
			{
				//("point touch released\n");
				input_report_key(input_dev, BTN_TOUCH, 0);
		...
		else
		{
			...
			for (i = 0; i < info.count; i ++) 
			{
				...
				input_report_abs(input_dev, ABS_MT_POSITION_X, info.point[i].x);
				input_report_abs(input_dev, ABS_MT_POSITION_Y, info.point[i].y);
...
#if defined(MSTAR_TP_GESTURE)
		if (( ts_data->gesture_id > 0) &&(1 == ts_data->gesture_on) ) {
			mstar_tp_resume(ts_data);
...
#if defined(MSTAR_TP_GESTURE)
		if ( ts_data->gesture_id > 0) {
			int ret = 0;
			enable_irq(ts_data->client->irq);
			ret = mstar_tp_suspend(ts_data);
			if (0 == ret) {
				mutex_unlock(&ts_data->ts_mutex);
				return 0;
			} else
...
static int msg21xx_ts_probe(struct i2c_client *client,
	...
	if (pdata->fw_name) {
		...
		if (len > FT_FW_NAME_MAX_LEN - 1) {
			printk("Invalid firmware name\n");
			return -EINVAL;
		}

		strlcpy(ts_data->fw_name, pdata->fw_name, len + 1);
	}
#endif
	...
	input_set_abs_params(input_dev, ABS_MT_POSITION_X, TOUCH_SCREEN_X_MIN, pdata->x_max, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_POSITION_Y, TOUCH_SCREEN_Y_MIN, pdata->y_max, 0, 0);
	/*
...
*/
#ifdef MSTAR_TP_GESTURE
	keyset_mstartp_gesture(input_dev);
#endif
...
#ifdef MSTAR_TP_GESTURE
	mstar_tp_gestures_register(ts_data);
#endif
	...
	ito_test_create_entry(i2c_client);
#endif
...
#ifdef MSTAR_PWRON_UPGRADE
	ret = mstar_init_update_proc(ts_data);
	if (ret < 0) {
		printk("Mstar TP Create firmware update thread error.\n");
	}
#endif
