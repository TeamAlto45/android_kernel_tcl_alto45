...
#define HI258_POP8S_SENSOR_NAME "hi258_pop8s"
#define PLATFORM_DRIVER_NAME "msm_camera_hi258_pop8s"
...
DEFINE_MSM_MUTEX(hi258_pop8s_mut);
static struct msm_sensor_ctrl_t hi258_pop8s_s_ctrl;

static struct msm_sensor_power_setting hi258_pop8s_power_setting[] = {
{
...
static struct msm_camera_i2c_reg_conf hi258_pop8s_preview_settings[] = {
{0x03, 0x00},
...
static struct msm_camera_i2c_reg_conf hi258_pop8s_snapshot_settings[] = {
/* 2M FULL Mode */
{0x03,0x00},
{0x01, 0x01},  // HI258_Sleep_Mode
...
#if 0
static struct msm_camera_i2c_reg_conf hi258_pop8s_start_settings[] = {
	{0x03, 0x00},
	{0x01, 0x00},
};

static struct msm_camera_i2c_reg_conf hi258_pop8s_stop_settings[] = {
	{0x03, 0x00},
	{0x01, 0x01},
};
#endif
...
static struct msm_camera_i2c_reg_conf hi258_pop8s_recommend_settings[] = {
{0x01, 0x01}, //sleep on
{0x01, 0x03}, //sleep off
{0x01, 0x01}, //sleep on
// PAGE 20
...
{0x6a, 0x5a}, //White Protection Offset Dark/Indoor
...
{0x22, 0x40}, //LSC R 1b->15 20130125
{0x23, 0x34}, //LSC G
{0x24, 0x34}, //LSC B
...
{0x84, 0xbd},
{0x85, 0x5e},
{0x86, 0x02}, //EXPMin 11428.57 fps
{0x87, 0x0d},
{0x88, 0x0c}, //EXP Max 60hz 7.50 fps
{0x89, 0x2d},
{0x8a, 0x30},
{0xa5, 0x0b}, //EXP Max 50hz 7.69 fps
{0xa6, 0xdf},
{0xa7, 0x42},
{0x8B, 0xe9}, //EXP100
{0x8C, 0xca},
{0x8D, 0xc2}, //EXP120
{0x8E, 0xd3},
{0x9c, 0x1c}, //EXP Limit 816.33 fps
{0x9d, 0xb6},
{0x9e, 0x02}, //EXP Unit
{0x9f, 0x0d},
...
static struct v4l2_subdev_info hi258_pop8s_subdev_info[] = {
	{
...
static const struct i2c_device_id hi258_pop8s_i2c_id[] = {
	{HI258_POP8S_SENSOR_NAME, (kernel_ulong_t)&hi258_pop8s_s_ctrl},
	{ }
};

static int32_t msm_hi258_pop8s_i2c_probe(struct i2c_client *client,
	   const struct i2c_device_id *id)
{
	   return msm_sensor_i2c_probe(client, id, &hi258_pop8s_s_ctrl);
}

static struct i2c_driver hi258_pop8s_i2c_driver = {
	.id_table = hi258_pop8s_i2c_id,
	.probe  = msm_hi258_pop8s_i2c_probe,
	.driver = {
		.name = HI258_POP8S_SENSOR_NAME,
	},
};

static struct msm_camera_i2c_client hi258_pop8s_sensor_i2c_client = {
	.addr_type = MSM_CAMERA_I2C_BYTE_ADDR,
};

static const struct of_device_id hi258_pop8s_dt_match[] = {
	{.compatible = "qcom,hi258_pop8s", .data = &hi258_pop8s_s_ctrl},
	{}
};

MODULE_DEVICE_TABLE(of, hi258_pop8s_dt_match);

static struct platform_driver hi258_pop8s_platform_driver = {
	.driver = {
		.name = "qcom,hi258_pop8s",
		.owner = THIS_MODULE,
		.of_match_table = hi258_pop8s_dt_match,
	},
};

static void hi258_pop8s_i2c_write_table(struct msm_sensor_ctrl_t *s_ctrl,
...
static int32_t hi258_pop8s_platform_probe(struct platform_device *pdev)
{
	int32_t rc;
	const struct of_device_id *match;
	match = of_match_device(hi258_pop8s_dt_match, &pdev->dev);
	rc = msm_sensor_platform_probe(pdev, match->data);
	return rc;
}

static int __init hi258_pop8s_init_module(void)
{
	int32_t rc;
	pr_info("%s:%d\n", __func__, __LINE__);
	rc = platform_driver_probe(&hi258_pop8s_platform_driver,
		hi258_pop8s_platform_probe);
	if (!rc)
		return rc;
	return i2c_add_driver(&hi258_pop8s_i2c_driver);
}

static void __exit hi258_pop8s_exit_module(void)
{
	pr_info("%s:%d\n", __func__, __LINE__);
	if (hi258_pop8s_s_ctrl.pdev) {
		msm_sensor_free_sensor_data(&hi258_pop8s_s_ctrl);
		platform_driver_unregister(&hi258_pop8s_platform_driver);
	} else
		i2c_del_driver(&hi258_pop8s_i2c_driver);
	return;
}

static int32_t hi258_pop8s_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	int32_t rc = 0;
	uint16_t chipid = 0;
	...
	if (rc < 0) {
		pr_err("%s: %s: hi258_pop8s read id failed\n", __func__,
...
int32_t hi258_pop8s_sensor_config(struct msm_sensor_ctrl_t *s_ctrl,
	void __user *argp)
{
	int32_t rc = 0;
	int32_t i = 0;
	...
	case CFG_SET_INIT_SETTING:
		CDBG("init setting");
		hi258_pop8s_i2c_write_table(s_ctrl,
				&hi258_pop8s_recommend_settings[0],
				ARRAY_SIZE(hi258_pop8s_recommend_settings));
		CDBG("init setting X");
		break;
	case CFG_SET_RESOLUTION: {
		...
		if (val == 0)
			hi258_pop8s_i2c_write_table(s_ctrl, &hi258_pop8s_snapshot_settings[0],
				ARRAY_SIZE(hi258_pop8s_snapshot_settings));
		else if (val == 1)
			hi258_pop8s_i2c_write_table(s_ctrl, &hi258_pop8s_preview_settings[0],
				ARRAY_SIZE(hi258_pop8s_preview_settings));
		break;
	}
...
/*[BUGFIX]-Mod-BEGIN by TCTNB.WPL, bug564473, 2013/11/30 */
	#if 0
		hi258_pop8s_i2c_write_table(s_ctrl,
			&hi258_pop8s_stop_settings[0],
			ARRAY_SIZE(hi258_pop8s_stop_settings));
	#endif
/*[BUGFIX]-Mod-END by TCTNB.WPL */
		break;
...
/*[BUGFIX]-Mod-BEGIN by TCTNB.WPL, bug564473, 2013/11/30 */
		hi258_pop8s_i2c_write_table(s_ctrl,
			&hi258_pop8s_snapshot_settings[0],
			ARRAY_SIZE(hi258_pop8s_snapshot_settings));
#if 0
		hi258_pop8s_i2c_write_table(s_ctrl,
			&hi258_pop8s_start_settings[0],
			ARRAY_SIZE(hi258_pop8s_start_settings));
#endif
/*[BUGFIX]-Mod-END by TCTNB.WPL */
		break;
	...
	case CFG_SET_EXPOSURE_COMPENSATION: {
		...
		pr_debug("%s: Exposure compensation Value is %d",
			__func__, ec_lev);
		//hi258_pop8s_set_exposure_compensation(s_ctrl, ec_lev);
		break;
	}
	case CFG_SET_WHITE_BALANCE: {
		...
		//hi258_pop8s_set_white_balance_mode(s_ctrl, wb_mode);
		break;
	}
	case CFG_SET_EFFECT: {
		int32_t effect_mode;
...
static struct msm_sensor_fn_t hi258_pop8s_sensor_func_tbl = {
	.sensor_config = hi258_pop8s_sensor_config,
	.sensor_power_up = msm_sensor_power_up,
	.sensor_power_down = msm_sensor_power_down,
	.sensor_match_id = hi258_pop8s_sensor_match_id,
};

static struct msm_sensor_ctrl_t hi258_pop8s_s_ctrl = {
	.sensor_i2c_client = &hi258_pop8s_sensor_i2c_client,
	.power_setting_array.power_setting = hi258_pop8s_power_setting,
	.power_setting_array.size = ARRAY_SIZE(hi258_pop8s_power_setting),
	.msm_sensor_mutex = &hi258_pop8s_mut,
	.sensor_v4l2_subdev_info = hi258_pop8s_subdev_info,
	.sensor_v4l2_subdev_info_size = ARRAY_SIZE(hi258_pop8s_subdev_info),
	.func_tbl = &hi258_pop8s_sensor_func_tbl,
};

module_init(hi258_pop8s_init_module);
module_exit(hi258_pop8s_exit_module);
