...
int s5k5e2_alto45_cmcc_RG_Gold = 870;//911;//202//0.792760725 * 256;// sync with GS
int s5k5e2_alto45_cmcc_BG_Gold = 640;//744;//161//0.627300368 * 256;// sync with GS
...
static int pub_R_gain = 0,pub_G_gain=0, pub_B_gain=0,data_ready =0;
	    ...
	    if(s5k5e2_alto45_cmcc_is_flag){// flag-s
            ...
            if(flag_count == 1){
                ...
                for(j=0; j<s5k5e2_alto45_cmcc_data_size; j++){
					...
					#if S5K5E2_ALTO45_CMCC_OTP_DEBUG_ON
                    printk("=====> [data]read addr:[0x%02x],data:%x\n", s5k5e2_alto45_cmcc_data_start+j,dest[j]);
					#endif
                }
				#if S5K5E2_ALTO45_CMCC_OTP_DEBUG_ON
...
static void s5k5e2_alto45_cmcc_OTP_all_calibration(struct msm_camera_i2c_client *i2c_client)
{
    //int ret1;
    if(actuator_exist){
		s5k5e2_alto45_cmcc_RG_Gold = 870;//202//0.792760725 * 256;// sync with GS
		s5k5e2_alto45_cmcc_BG_Gold = 640;//161//0.627300368 * 256;// sync with GS
	}
	else{
		s5k5e2_alto45_cmcc_RG_Gold = 841;//202//0.792760725 * 256;// sync with GS
		s5k5e2_alto45_cmcc_BG_Gold = 692;//161//0.627300368 * 256;// sync with GS
    }
	if(need_check)
		s5k5e2_alto45_cmcc_check_normal_data(i2c_client);
	if(data_ready)
		s5k5e2_alto45_cmcc_update_awb_gain(pub_R_gain, pub_G_gain, pub_B_gain);
	else
		s5k5e2_alto45_cmcc_update_otp_wb(i2c_client);
#if S5K5E2_ALTO45_CMCC_OTP_DEBUG_ON
	printk("pub_R:%d, pub_G:%d, pub_B:%d\n",pub_R_gain, pub_G_gain, pub_B_gain);
#endif
}

void S5K5E2_ALTO45_CMCC_TCT_OTP_get_data(struct msm_camera_i2c_client *i2c_client)
{
	if(!data_ready){
    g_client = i2c_client;
