...
#define BMA2x2_VDD_MIN_UV       1750000
#define BMA2x2_VDD_MAX_UV       1950000
...
static void bma2x2_set_enable(struct device *dev, int enable)
{
	...
	if (enable) {
		if (pre_enable == 0) {
			...
			enable_irq(bma2x2->bma2x2_client->irq);
		}

	} else {
		if (pre_enable == 1) {
			disable_irq(bma2x2->bma2x2_client->irq);
		...
		const struct i2c_device_id *id)
{
	...
	disable_irq(data->bma2x2_client->irq);
	...
	data->cdev.delay_msec = data->pdata->poll_interval;
